    ...
    for (x, y, w, h) in faces:
        ...
        for i, value in enumerate(MemberList):
            if pred == 0:
                ...
                cv2.putText(frame,first,(x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(value.B, value.G, value.R),2,cv2.CV_AA)
                cv2.putText(frame,second,(x, y + h + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(value.B, value.G, value.R),2,cv2.CV_AA)
                cv2.putText(frame,third,(x, y + h + 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(value.B, value.G, value.R),2,cv2.CV_AA)
